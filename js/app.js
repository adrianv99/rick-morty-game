...
const initApp = async () => {
    try {
        ...
        const { results: aliveCharacters } = await api.fetchCharacters({ status: 'alive' });
        const { results: deadCharacters } = await api.fetchCharacters({ status: 'dead' });

        // shuffle characters
        characters = [].concat(aliveCharacters, deadCharacters).sort((a, b) => 0.5 - Math.random());
